...
export function ThemeToggle({ compact }: Readonly<{ compact?: boolean }>) {
    ...
    if (compact) {
        return (
            <button
                onClick={toggle}
                className='flex items-center gap-2 px-2 py-1.5 text-sm text-muted-foreground hover:text-foreground rounded-lg hover:bg-muted transition-colors'
    ...
    return (
        <button
            ...
            aria-checked={isDark}
            aria-label={t.nav.toggleTheme}
            className='flex items-center gap-2 px-2 py-1.5 text-sm text-muted-foreground hover:text-foreground rounded-lg hover:bg-muted transition-colors w-full'>
            {isDark ? (
                <Moon className='w-4 h-4 shrink-0' />
            ) : (
                <Sun className='w-4 h-4 shrink-0' />
