...
- **The empty community.** A fresh deployment has no Activities and no Sessions,
  so the real-data bands (04) render empty on a page whose whole job is to
  convince. `PRODUCT.md:103` says empty states are the default state. Sharpness
  waits on 04 deciding which data appears at all.
...
- **Performance of an unauthenticated DB read on `/`.** Public traffic hitting
  Prisma on a 1-connection serverless pool (`PRODUCT.md:72`) is a real risk once
  04 says what gets queried. Caching strategy unknown until then.
