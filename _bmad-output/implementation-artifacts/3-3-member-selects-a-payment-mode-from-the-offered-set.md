...
- [x] [Review][Patch] Stale standing-mode read lets a re-picked mode retroactively rewrite an already-effective period, violating AD-7/AC3 immutability [src/app/api/users/memberships/[ekskulId]/mode/route.ts:97-107] — `resolveSwitch` compares the requested mode against the raw DB `paymentMode` field, but `resolvePaymentMode` (src/lib/payment-mode.ts:108-114) already treats `pendingMode` as effective once `pendingEffectiveFrom` has arrived, without ever writing that graduation back to `paymentMode`/`effectiveFrom`. Once a queued switch's period arrives, re-selecting the old standing mode is read as "cancel pending" instead of "change from the now-effective pending mode", silently flipping what the current, already-in-progress period resolves to. Fix: before calling `resolveSwitch`, resolve the true current-period mode via `resolvePaymentMode` (fetch `pendingMode`/`pendingEffectiveFrom`/`effectiveFrom` alongside `paymentMode`, per Task 3's own spec) and compare against that, not the raw `paymentMode` field.
- [x] [Review][Patch] Mode-switch route omits `isActive` scoping [src/app/api/users/memberships/[ekskulId]/mode/route.ts:83-101] — both the `ekskul.findUnique` and the re-fetched `membership.findUnique` skip `isActive`, unlike the sibling GET `/api/users/memberships` route, which filters both. A member can mutate their payment mode on a deactivated Activity, or after having left (if the leave and the PATCH race), leaving stale mode fields that resurface if they rejoin.
- [x] [Review][Patch] Read-then-write race with no transaction guard [src/app/api/users/memberships/[ekskulId]/mode/route.ts:97-108] — the standing-mode read and the `membership.update` are two separate round trips with no `$transaction`/optimistic-concurrency guard. Concurrent PATCH calls (double-click, multi-tab/device) can race and one write silently clobbers the other's queued state.
- [x] [Review][Patch] `fetchMemberships()`'s blanket `.catch(() => [])` is reused for post-mutation `refresh()` [src/app/(main)/profile/ekskul-memberships.tsx:28-51] — a transient network/parse failure on the re-fetch after a *successful* join/leave/mode-change wipes the member's entire visible activities list, a regression from the prior code's optimistic local-state update.
- [x] [Review][Patch] `saving` state is cleared before the fire-and-forgotten refresh resolves [src/app/(main)/profile/payment-mode-selector.tsx:120-129] — `onChanged()` (→ `refresh()`) is not awaited, so `finally { setSaving(null) }` re-enables the mode cards before the refreshed effective/pending state has actually landed, opening a stale-click window.
- [x] [Review][Patch] Unhandled JSON parse failure in the PATCH route [src/app/api/users/memberships/[ekskulId]/mode/route.ts:75] — `await req.json()` has no try/catch; a malformed or empty body throws uncaught and returns a generic 500, inconsistent with the handler's otherwise-consistent 400 contract for every other validation failure.
- [x] [Review][Patch] Admin member-detail hardcodes Indonesian-only labels instead of this story's own dictionary keys [src/app/(admin)/admin/members/[id]/page.tsx:13,138] — `PAYMENT_MODE_LABELS: Record<string, string>` duplicates `t.paymentMode.monthly`/`t.paymentMode.perSession` (added by Task 7 for exactly this purpose) with hardcoded Indonesian strings, so an English-locale admin still sees "Bulanan"/"Per sesi". Violates Task 7's "no hardcoded strings" rule and NFR-6 parity; also weakly typed (`Record<string, string>` instead of `Record<PaymentMode, string>`) so a typo or future third mode would silently render blank.
- [x] [Review][Patch] AC3's required "note echoing the queued mode" omits the mode name [src/app/(main)/profile/payment-mode-selector.tsx:160] — the pending-switch note shows only the period (`t.paymentMode.pendingNote` + period label), never which mode (Monthly/Per-Session) is queued; the `ModeCard` selection ring also never reflects the pending target, so a member has no visual cue which mode is about to take effect.
- [x] [Review][Patch] Dead/unused i18n keys `paymentMode.title` and `paymentMode.changeCta` [src/lib/i18n/dictionaries.ts:360,371,814,825] — added to both `en`/`id` but never referenced by any component in this diff (the selector uses `choosePrompt`, not `title`; nothing renders a "Change" CTA).
- [x] [Review][Patch] `SingleModeLine` silently renders nothing when an Activity offers neither mode [src/app/(main)/profile/payment-mode-selector.tsx:77-90] — no fallback message if `effectiveMode` is null on the single-mode branch. Likely unreachable today given the existing "≥1 payment mode" validation on Ekskul create/update, but defensive-only, not enforced by this diff.
- [x] [Review][Patch] Task 4's GET response omits `paymentMode`/`effectiveFrom` despite fetching and spec'ing both [src/app/api/users/memberships/route.ts:25,74-75,86] — `toEkskulView`'s return object drops both fields; no visible symptom today (the UI relies on `effectiveMode`), but a literal deviation from Task 4's explicit field list.
- [x] [Review][Patch] AD-12 style violation: bare string literals typed as `PaymentMode` instead of enum member access [src/app/(main)/profile/payment-mode-selector.tsx:10] — `import type { PaymentMode }` + `const MONTHLY: PaymentMode = 'MONTHLY'` instead of a value import + `PaymentMode.MONTHLY`, unlike the PATCH route which does this correctly.
- [x] [Review][Patch] Busy state not applied to the sibling `ModeCard` during an in-flight PATCH [src/app/(main)/profile/payment-mode-selector.tsx:146,154] — `busy` only compares against the specific mode being saved, so the other card still renders fully interactive while a PATCH is in flight. Cosmetic only — `choose()`'s top-level `if (saving || isNoOp) return` already no-ops any click regardless.
- [x] [Review][Defer] Timezone assumption in `currentPeriod`/`nextPeriod` [src/lib/payment-mode.ts:36-49] — deferred, pre-existing. Period math derives from server-local `getMonth()`/`getFullYear()`; on typical UTC serverless infra serving an Indonesia (UTC+7) audience, requests near local midnight at month boundaries could resolve to the wrong billing period. No project convention yet exists for pinning "now" to a fixed timezone — pick up when timezone handling is established project-wide (affects Story 3.4/3.5 billing-period math too, not unique to this story's new helpers).
