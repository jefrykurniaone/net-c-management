...
Reviewed in 5 diff chunks (5168 total diff lines, over the ~3000-line single-pass threshold): **A** tokens/badge/utils/4 shared components/i18n, **B** shells/nav, **C** member screens, **D** admin screens, **E** landing/auth/misc. 3 adversarial layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) per chunk, cross-checked against the live codebase (not just the diff) before triage.

- [x] [Review][Decision] `ekskul-actions.tsx` `maxPlayers` input coercion was changed (empty→`undefined` instead of the old `Number.parseInt(...) || 0` masking) without a NaN guard for non-numeric input — conflicts with this story's own scope note ("if you happen to be editing that file for colors, do NOT also change its validation logic") though it does fulfill an older Story 2.2 deferred-work request to align this during Epic 4. [`src/app/(admin)/admin/ekskul/ekskul-actions.tsx`] — **Resolution: finished the fix.** Added a unified `Number.isNaN(parsed)` guard so both empty and non-numeric input map to `undefined` — no more possible `NaN` in form state.

- [x] [Review][Patch] `badge.tsx`'s new `success`/`warning` variants had no color-matched `focus-visible:ring-*`, unlike `destructive`'s sibling treatment — inconsistent with the story's own "one visual family" framing for the three payment badges. [`src/components/ui/badge.tsx`] — **Fixed:** added `focus-visible:ring-success/20 dark:focus-visible:ring-success/40` and the `warning` equivalent.
- [x] [Review][Patch] `paymentStatusVariant`/`roleBadgeVariant` (`src/lib/utils.ts`) took loose `string` params instead of the `PaymentStatus`/`Role` Prisma enums already used elsewhere in the codebase, losing exhaustiveness checking. [`src/lib/utils.ts`] — **Fixed:** narrowed both params to their Prisma enum types. This surfaced one real latent bug: `src/app/(main)/profile/page.tsx`'s hand-rolled `Profile` interface declared `role: string` instead of `Role`, which the new stricter typing caught at build time — fixed by importing `Role` as a type there too. `npm run build` verified green after.
- [x] [Review][Patch] `payments/upload/page.tsx` (×2) and `sessions/[id]/pay/page.tsx` (×1) hand-rolled their loading/error/not-found empty states instead of using the shared `<EmptyState>` component, contradicting this story's own Task 5 instruction for these exact files and AC1's "no screen hand-rolls ... an empty state" rule. [`src/app/(main)/payments/upload/page.tsx`, `src/app/(main)/sessions/[id]/pay/page.tsx`] — **Fixed:** all 3 call sites now use `<EmptyState title={...} />`.

- [x] [Review][Defer] `CommunityIdentityMark`'s `logoUrl` branch has no `onError` fallback to the abbreviation mark if the image fails to load — fixing it requires a client-side event handler, which conflicts with this story's explicit "server-safe" requirement for the four new shared components. Architectural tradeoff, not a quick patch. [`src/components/community/identity-mark.tsx`] — deferred, pre-existing class of issue (same as 4.2's avatar-fallback defer)
- [x] [Review][Defer] Name→initials computation is duplicated verbatim across `member-nav.tsx`/`mobile-nav.tsx`/`sidebar.tsx` — pre-existing duplication, not introduced by this token-only diff; a shared-hook extraction would itself be scope creep for a "visual refresh only" story. [`src/components/layout/*-nav.tsx`, `sidebar.tsx`] — deferred, pre-existing
- [x] [Review][Defer] Sign-out buttons in all three nav shells have no disabled/pending guard against a rapid double-click firing duplicate `signOut()` calls — pre-existing gap, not touched by this diff's color migration. [`src/components/layout/member-nav.tsx`, `mobile-nav.tsx`, `sidebar.tsx`] — deferred, pre-existing

**Dismissed as noise / verified false / pre-existing-unchanged / out-of-4.3-scope (~110 across all 5 chunks):** Group A — oklch/hex token mix (cosmetic, spec-mandated exact hex values), missing `dark:text-success/warning` override (verified false: the CSS custom property itself already swaps under `.dark`, no override needed), `communityAbbr` "dangling reference" (verified false: exists in `utils.ts:31`), i18n key-removal "breaking risk" (verified false: zero dangling references found via repo-wide grep), `mobile-card` `accentColor` "breaks semantic-token discipline" (explicitly spec-sanctioned exception — runtime activity color). Group B — "admin nav leaks to non-admin members" (verified **false**: `Sidebar`/`MobileNav` only ever mount inside the already-role-gated `(admin)/layout.tsx`; `MemberTopBar`/`MemberBottomNav` are the separate member-shell components, from Story 4.1's shell split), landmark/aria-label nits (pre-existing from Story 4.1, untouched by this token diff). Groups A/C/D — several large functional changes (`rsvp-button.tsx`'s per-session payment logic, `sessions/[id]/page.tsx` + `sessions/[id]/pay/page.tsx`'s payment-mode resolution, `payments/upload/page.tsx`'s server-authoritative-fee rewrite, `payment-actions.tsx`'s UX-DR12 reject-reason `window.prompt()` flow, `admin/settings/page.tsx`'s `maxPlayers` field removal) are Story 3.3/3.4/3.5/4.4 business logic bundled into the same giant commit (`347770b`) — all already reviewed/closed under their own stories, out of scope for this review. Group D — OWNER-badge/StatCard-icon/attendance-color "regressions" all verified **false** or matching explicitly-documented, audited fixes already in the Dev Agent Record (payment-badge `paymentStatusVariant` reuse, attendance-toggle money-state-leak fix). Group E — fully clean, zero findings from any layer.
