...
- **The admission gate.** Applicant, Admit / Decline, a waiting page, an admin queue with a count badge, and the admission email. Until it lands, signing in still joins, and the deactivation control on the member roster still writes a flag nothing reads.
- **Page metadata addressed to a stranger.** The public page's title is the community name alone and its description is its own string that may not promise inventory, so it stays true for a community with nothing scheduled yet. The link-preview image is generated from the community name; there is no static brand asset to use and none is wanted.
