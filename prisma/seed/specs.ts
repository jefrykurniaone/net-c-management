...
export const SCENARIO_SPECS: Record<string, SessionSpec> = {
    ...
    holdLab: { key: 'holdLab', slug: 'badminton', title: 'Hold Lab (Per-Session Test)', date: addDays(wibDayStart(now), 4), startTime: '18:00', endTime: '20:00', location: 'GOR Cempaka Court 4', maxPlayers: 8, fee: 25_000 },
    // Every seat funded and taken → tests the "Full" state as Adi (not attending).
    full: { key: 'full', slug: 'badminton', title: 'Full Court Challenge', date: addDays(wibDayStart(now), 5), startTime: '19:00', endTime: '21:00', location: 'GOR Cempaka Court 5', maxPlayers: 6, fee: 25_000 },
    // 2 going < minMembers (4) → admin "remind members" flow, lastReminderAt null.
    underbooked: { key: 'underbooked', slug: 'futsal', title: 'Underbooked Friendly', date: addDays(wibDayStart(now), 8), startTime: '20:00', endTime: '22:00', location: 'Champions Arena A', maxPlayers: 12, fee: 15_000 },
    // Cancelled session with attendees → tests CANCELLED rendering + no-register.
    cancelled: { key: 'cancelled', slug: 'badminton', title: 'Rained Out (Cancelled)', date: addDays(wibDayStart(now), 3), startTime: '19:00', endTime: '21:00', location: 'GOR Cempaka Court 3', maxPlayers: 24, fee: 25_000, status: SessionStatus.CANCELLED },
    // Happening right now (anchor day) → tests the ONGOING state.
    ongoing: { key: 'ongoing', slug: 'basket', title: 'Live Pickup (Ongoing)', date: wibDayStart(now), startTime: '19:30', endTime: '21:30', location: 'GBK Basketball Hall', maxPlayers: 10, fee: 20_000, status: SessionStatus.ONGOING },
    // Scheduled today (WIB), dayReminderSentAt = null → target for the day-reminder
    // cron, whose WIB-day window matches on dates stored as UTC midnight. Every
    // spec above now uses the same wibDayStart for the same reason.
